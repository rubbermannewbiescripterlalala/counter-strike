...
new const WEAPONNAMES[][] = { "", "\yP228 Compact", "", "\ySchmidt Scout", "", "\rXM1014 M4", "", "\rIngram MAC-10", "\rSteyr AUG A1",
			"", "\rDual Elite Berettas", "\rFiveseveN", "\rUMP 45", "\rSG-550 Auto-Sniper", "\rIMI Galil", "\rFamas",
			"\rUSP .45 ACP Tactical", "\rGlock 18C", "\rAWP Magnum Sniper", "\rMP5 Navy", "\rM249 Para Machinegun",
			"\rM3 Super 90", "\rM4A1 Carbine", "\rSchmidt TMP", "\rG3SG1 Auto-Sniper", "", "\rDesert Eagle .50 AE",
			"\rSG-552 Commando", "\rAK-47 Kalashnikov", "", "\rES P90" }
	...
	{ 
		...
		{ 
			...
			else
			{
				...
				set_task( 1.0 , "deadmau5_respawn_now", id)
...
// Respawn now!
public deadmau5_respawn_now(id)
{
	...
	if (!is_user_alive(id))
	{
		...
		if(g_nemround)
		{
			set_task( 7.5 , "deadmau5_welcome_text", id)
		}
		else if(g_survround)
		{
			set_task( 7.5 , "deadmau5_welcome_text", id)
...
public deadmau5_welcome_text(id)
{
	set_hudmessage(0, 125, 0, -1.0, 0.17, 1, 2.0, 2.0, 4.0, 2.0, -1)
	ShowSyncHudMsg(id, g_MsgSync, "Sky Arena ZM vam zeli ugodnu igru!^n IP Servera: 193.192.59.74:27111")
}
...
show_menu_game(id)
{
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\r%s^n^n", g_modname)
	...
	if (get_pcvar_num(cvar_zclasses))
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\r %L^n", id,"MENU_ZCLASS")
	else
	...
	//7. Vip menu 
	if (g_isalive[id])
	         len += formatex(menu[len], charsmax(menu) - len, "\y7.\r VIP Menu^n^n")
	else
	         len += formatex(menu[len], charsmax(menu) - len, "\r7.\d VIP Menu^n^n") 	
	...
	switch (key)
	{
		...
		case 5: // Join Spectator
		{
			...
			fm_user_team_update(id)
                  ...
                  case 6: {
		if (g_isalive[id])
		client_cmd(id, "say /vm"); // VIP menu
		else 
		        zp_colored_print(id, "^x03[VIP Menu]^x01 %L", id, "CMD_NOT")
		}
		case 8: // Admin Menu
		{
...
humanme(id, survivor, silentmode)
{
	...
	else
	{
		...
		if (!silentmode)
		{
			...
			client_print(0, print_chat, "%s got saved by a miracle.", g_playername[id])
		}
	}
